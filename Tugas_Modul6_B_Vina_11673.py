...
def custom_progress_bar(confidence, color1, color2):
    ...
    <div style="border: 1px solid #ddd; border-radius: 5px; overflow: hidden; width: 100%; font-size: 14px;">
        <div style="width: {percentage1}%; background: {color1}; color: white; text-align: center; height: 24px; float: left;">
            {percentage1:.2f}%
        </div>
        <div style="width: {percentage2}%; background: {color2}; color: white; text-align: center; height: 24px; float: left;">
            {percentage2:.2f}%
        </div>
    </div>
    """
...
# Streamlit UI
st.title("Prediksi Kematangan Buah Naga - 1673")  # 4 digit NPM terakhir
...
# Sidebar prediction button and results
